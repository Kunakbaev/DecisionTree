...
static DecisionTreeErrors askQuestion(DecisionTree* tree, const Node* node, bool* isToLeftSon) {
    IF_ARG_NULL_RETURN(tree);
...
static DecisionTreeErrors isObjectGuessedCorrectly(DecisionTree* tree, const Node* node, bool* isCorrectGuess) {
    IF_ARG_NULL_RETURN(tree);
    IF_ARG_NULL_RETURN(node);
...
static DecisionTreeErrors addNewObjectToDecisionTree(DecisionTree* tree, Node* parent, Node* wronglyGuessedObj) {
    IF_ARG_NULL_RETURN(tree);
    ...
    size_t bruh = wronglyGuessedObj->memBuffIndex; // you found an easter egg
...
static DecisionTreeErrors recursiveSaveOfTreeInFile(DecisionTree* tree, size_t nodeInd, size_t depth, FILE* file) {
    if (!nodeInd)
        return DECISION_TREE_STATUS_OK;
...
static DecisionTreeErrors getCommonPathCntArray(DecisionTree* tree, const char* objName1, const char* objName2,
                                                size_t** cntArr) {
    IF_ARG_NULL_RETURN(tree);
...
DecisionTreeErrors dumpCommonPathOf2Objects(DecisionTree* tree, const char* objName1, const char* objName2) {
    ...
    size_t* cntArr = NULL;
    IF_ERR_RETURN(getCommonPathCntArray(tree, objName1, objName2, &cntArr));
...
static DecisionTreeErrors terminalCmdHelp(DecisionTree* tree) {
    IF_ARG_NULL_RETURN(tree);
...
static DecisionTreeErrors terminalCmdPrintDefinition(DecisionTree* tree) {
    IF_ARG_NULL_RETURN(tree);
...
static DecisionTreeErrors terminalCmdReadFromFile(DecisionTree* tree) {
    IF_ARG_NULL_RETURN(tree);
...
static DecisionTreeErrors terminalCmdSaveToFile(DecisionTree* tree) {
    IF_ARG_NULL_RETURN(tree);
...
static DecisionTreeErrors commonOrDifferenceHelperFunc(DecisionTree* tree, bool isShowSame) {
    IF_ARG_NULL_RETURN(tree);
...
static DecisionTreeErrors terminalCmdShowCommonOf2Objects(DecisionTree* tree) {
    IF_ERR_RETURN(commonOrDifferenceHelperFunc(tree, true));
    return DECISION_TREE_STATUS_OK;
}

static DecisionTreeErrors terminalCmdShowDifferenceOf2Objects(DecisionTree* tree) {
    IF_ERR_RETURN(commonOrDifferenceHelperFunc(tree, false));
    return DECISION_TREE_STATUS_OK;
}

static DecisionTreeErrors terminalCmdShowDifferenceImage(DecisionTree* tree) {
    IF_ARG_NULL_RETURN(tree);
...
static DecisionTreeErrors executeCommand(DecisionTree* tree, const char* commandName, bool* isQuit) {
    IF_ARG_NULL_RETURN(tree);
    ...
    for (size_t commandInd = 0; commandInd < NUM_OF_COMMANDS; ++commandInd) {
        ...
        if (strcmp(command.cmdName, commandName) == 0) {
            DecisionTreeErrors error = (*command.funcPtr)(tree);
            IF_ERR_RETURN(error);
        }
    }
